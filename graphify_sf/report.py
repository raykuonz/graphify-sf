# generate GRAPH_REPORT.md - the human-readable audit trail for Salesforce metadata
...
def generate(
    ...
    lines = [
        f"# Salesforce Metadata Graph Report - {root}  ({today})",
        "",
        "## Corpus Check",
    ]
    if detection_result.get("warning"):
        ...
        total_files = detection_result.get("total_files", G.number_of_nodes())
        lines += [
            f"- {total_files} SF metadata files scanned",
            "- Verdict: graph structure adds value for navigating cross-component dependencies.",
        ]
    ...
    # SF metadata type coverage
    sf_type_counts: Counter = Counter()
    file_type_counts: Counter = Counter()
    for _, data in G.nodes(data=True):
        sf_type = data.get("sf_type", "Unknown")
        file_type = data.get("file_type", "unknown")
        if sf_type:
            sf_type_counts[sf_type] += 1
        if file_type:
    ...
    if built_at_commit:
        lines += [
            ...
            "- Run `graphify-sf <path> --update` after metadata changes.",
        ]
    ...
    lines += [
        "",
        "## God Nodes (most connected - your core SF abstractions)",
    ]
    for i, node in enumerate(god_node_list, 1):
        sf_type = node.get("sf_type", "")
        type_tag = f" ({sf_type})" if sf_type else ""
        lines.append(f"{i}. `{node['label']}`{type_tag} - {node['degree']} edges")

    lines += ["", "## Surprising Connections (cross-component dependencies)"]
    if surprise_list:
        for s in surprise_list:
            ...
            lines += [
                ...
                f"  {files[0]} → {files[1]}" + (f"  _{why or note}_" if (why or note) else ""),
            ]
    else:
